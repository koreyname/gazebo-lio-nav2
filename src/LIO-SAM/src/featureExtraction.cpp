/****************************************************************************
*************step2-----featureExtraction(特征点提取)***************************

功能：
+ 对经过运动畸变校正之后的当前帧激光点云，计算每个点的曲率（每一行的最前和最后的五个点不计算），
...
发布：
  包含提取完特征点的点云信息包。在订阅的信息包基础上塞进去提取出的特征角点和特征平面点
...
public:
    ...
    rclcpp::Subscription<lio_sam_op::msg::CloudInfo>::SharedPtr subLaserCloudInfo_;
    ...
    // 当前激光帧提取的平面点点云
    pcl::PointCloud<PointType>::Ptr surfaceCloud_;
    ...
    // 这是一个N_SCAN*Horizon_SCAN长的数组:cloudNeighborPicked_ = new int[N_SCAN*Horizon_SCAN];
        ...
        : ParamServer("lio_sam_featureExtraction", options)
    {
        ...
        subLaserCloudInfo_ = create_subscription<lio_sam_op::msg::CloudInfo>(
    ...
    void initializationValue()
    {
        ...
        surfaceCloud_.reset(new pcl::PointCloud<PointType>());
    ...
    /**
     * @brief
     * 去畸变后点云订阅的回调函数，业务流程如下：
     * 1. 接收到从imageProjection中发布出的一个去畸变点云信息cloudInfo(自定义格式)
     * 2. 对每个点计算曲率。计算周围点的平均距离用来作为曲率的替代
     * 3. 标记遮挡点和与激光平行的点，后续这些点不能采纳为特征点
     * 4. 特征提取。分别做角点（曲率大）和平面点（曲率小）特征点提取
     * 5. 整合信息，发布完整数据包
     *
    ...
    void laserCloudInfoHandler(const lio_sam_op::msg::CloudInfo::SharedPtr msgIn)
    {
        ...
        cloudInfo_ = *msgIn;
        cloudHeader_ = msgIn->header;
        // 把ros2 PointCloud2转成PCL格式，方便后面处理
        pcl::fromROSMsg(msgIn->cloud_deskewed, *extractedCloud_);
    ...
    {
        int cloudSize = extractedCloud_->points.size();
        for (int i = 5; i < cloudSize - 5; i++)
        {
            float diffRange = cloudInfo_.point_range[i - 5] +
                              cloudInfo_.point_range[i - 4] +
                              cloudInfo_.point_range[i - 3] +
                              cloudInfo_.point_range[i - 2] +
                              cloudInfo_.point_range[i - 1] -
                              cloudInfo_.point_range[i] * 10 +
                              cloudInfo_.point_range[i + 1] +
                              cloudInfo_.point_range[i + 2] +
                              cloudInfo_.point_range[i + 3] +
                              cloudInfo_.point_range[i + 4] +
                              cloudInfo_.point_range[i + 5];
     ...
     * 对于遮挡的物体，会呈现一条类似边缘的线。对于遮挡的点，其与左边或者右边的点会呈现断层状态。
       因此可以通过range的差距判断
    ...
    void markOccludedPoints()
    {
        ...
        {
            // 标记被遮挡的点
            float depth1 = cloudInfo_.point_range[i];
            float depth2 = cloudInfo_.point_range[i + 1];
            int columnDiff = std::abs(int(cloudInfo_.point_col_ind[i + 1] -
                                          cloudInfo_.point_col_ind[i]));
            // 两个点的列索引相差10个像素之内，认为是同一块区域
            if (columnDiff < 10)
            {
                if (depth1 - depth2 > 0.3) // 当前点距离大于右点距离0.3米，认为当前点及左边6个点无效
                {
                ...
                else if (depth2 - depth1 > 0.3) // 当前点距离小于右边点距离0.3米，认为右边6个点无效
                {
                    cloudNeighborPicked_[i + 1] = 1;
                    cloudNeighborPicked_[i + 2] = 1;
                    cloudNeighborPicked_[i + 3] = 1;
            ...
            // 标记平行与激光束的点
            float diff1 = std::abs(float(cloudInfo_.point_range[i - 1] - cloudInfo_.point_range[i]));
            float diff2 = std::abs(float(cloudInfo_.point_range[i + 1] - cloudInfo_.point_range[i]));
            // 当前点与左右两点的距离均大于阈值，认为当前点是处于平行面的点
            if (diff1 > 0.02 * cloudInfo_.point_range[i] && diff2 > 0.02 * cloudInfo_.point_range[i])
                cloudNeighborPicked_[i] = 1;
        }
    }
     ...
     *      1.3 cloudNeighborPicked_=1标记该点已进行特征提取
     *      1.4 对该角点左右各5个点，如果两点之间的列索引差距小于10，则抛弃周围的点，
     *          避免重复对同一块区域提取角点
     *      1.5 将该角点加入角点点云集合
     * 2. 提取平面点
     *      2.1 不限制每段平面点的数量，后面会进行降采样
     *      2.2 设置cloudLabel标志，cloudLabel=-1标志该点为平面点
     *      2.3 cloudNeighborPicked_=1标记该点已进行特征提取
     *      2.3 对该平面点左右各5个点，如果两点之间的列索引差距小于10，则抛弃周围的点，
     *          避免重复对同一块区域提取角点
    ...
    void extractFeatures()
    {
        ...
        for (int i = 0; i < N_SCAN; i++)
        {
            ...
            for (int j = 0; j < 6; j++)
            {
                ...
                int sp = (cloudInfo_.start_ring_index[i] * (6 - j) + cloudInfo_.end_ring_index[i] * j) / 6;
                int ep = (cloudInfo_.start_ring_index[i] * (5 - j) + cloudInfo_.end_ring_index[i] * (j + 1)) / 6 - 1;
                ...
                // 由于cloudSmoothness是从小到大排序，这里从ep（end point）到sp（start point）遍历，
                // 就是从大到小遍历。曲率大的点更有可能是边缘点和角点。
                for (int k = ep; k >= sp; k--)
                {
                    int ind = cloudSmoothness_[k].ind;
                    // cloudNeighborPicked_ == 1表示是被遮挡、平行或者已经被当作特征点的点
                    // 曲率大于edgeThreshold则认为是角点，默认=1.0
                    if (cloudNeighborPicked_[ind] == 0 && cloudCurvature_[ind] > edgeThreshold)
                    {
                        largestPickedNum++;
                        // 对于每一段，最多只提取20个角点
                        if (largestPickedNum <= 20)
                        {
                            // cloudLabel_ = 1 标志为角点，-1标志为平面点
                            cloudLabel_[ind] = 1;
                            cornerCloud_->push_back(extractedCloud_->points[ind]);
                        }
                        else
                        {
                            break;
                        ...
                        // 对该角点左右各5个点，如果两点之间的列索引差距小于10，
                        // 则抛弃周围的点，避免重复对同一块区域提取角点
                        for (int l = 1; l <= 5; l++)
                        {
                            int columnDiff = std::abs(int(cloudInfo_.point_col_ind[ind + l] -
                                                          cloudInfo_.point_col_ind[ind + l - 1]));
                            if (columnDiff > 10)
                                break;
                ...
                for (int k = sp; k <= ep; k++)
                {
                        ...
                        cloudCurvature_[ind] < surfThreshold)
                    {
                        ...
                        // 对该平面点左右各5个点，如果两点之间的列索引差距小于10，则抛弃周围的点，
                        // 避免重复对同一块区域提取角点
                        for (int l = 1; l <= 5; l++)
                        {
                            int columnDiff = std::abs(int(cloudInfo_.point_col_ind[ind + l] -
                ...
                for (int k = sp; k <= ep; k++)
                {
                    if (cloudLabel_[k] <= 0)
                    {
                        surfaceCloudScan->push_back(extractedCloud_->points[k]);
                    }
                }
            }
            ...
            *surfaceCloud_ += *surfaceCloudScanDS;
        }
    }
    ...
    {
        cloudInfo_.start_ring_index.clear();
        cloudInfo_.end_ring_index.clear();
        cloudInfo_.point_col_ind.clear();
        cloudInfo_.point_range.clear();
    }
    ...
    void publishFeatureCloud()
    {
        ...
        pclPointcloud2Ros(cornerCloud_, cloudInfo_.cloud_corner, cloudHeader_.stamp, lidarFrame);
        pclPointcloud2Ros(surfaceCloud_, cloudInfo_.cloud_surface, cloudHeader_.stamp, lidarFrame);
        // 发布cloudInfo数据包
