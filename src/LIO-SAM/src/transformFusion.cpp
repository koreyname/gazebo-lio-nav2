...
public:
    ...
    void lidarOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        try
        {
            const auto lidarToBaseMsg = tfBuffer->lookupTransform(
                lidarFrame, baseLinkFrame, tf2::TimePointZero);
            const Eigen::Isometry3d lidarToBase =
                tf2::transformToEigen(lidarToBaseMsg.transform);
            const Eigen::Isometry3d mapToBase = lidarOdomAffine * lidarToBase;

            auto tfMapToBase = tf2::eigenToTransform(mapToBase);
            tfMapToBase.header.stamp = odomMsg->header.stamp;
            tfMapToBase.header.frame_id = odomFrame;
            tfMapToBase.child_frame_id = baseLinkFrame;
            tfBroadcaster->sendTransform(tfMapToBase);
        }
