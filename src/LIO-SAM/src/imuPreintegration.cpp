/****************************************************************************
*************step3-----imuPreintegration（IMU预积分）*************************

功能：
+ 主要功能是订阅激光里程计（低频里程计，来自mapOptimization)，对两帧激光里程计中间过程使用IMU信号做积分，
  得到高频的IMU里程计（与IMU同频）
...
2. 订阅IMU原始数据
*  2.1 加锁，对新来的IMU数据放入两个队列（imuQueOpt,imuQueImu_)
...
public:
    ...
    rclcpp::CallbackGroup::SharedPtr callbackGroupOdom_;
    ...
    std::deque<sensor_msgs::msg::Imu> imuQueOpt_;
    // imuQueImu用来给imuIntegratorImu_提供数据来源。优化当前激光里程计之后，下一帧激光里程计到来之前的位姿
    std::deque<sensor_msgs::msg::Imu> imuQueImu_;
    ...
    gtsam::Pose3 imu2Lidar_ = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(-extTrans.x(), -extTrans.y(), -extTrans.z()));
    gtsam::Pose3 lidar2Imu_ = gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(extTrans.x(), extTrans.y(), extTrans.z()));
    ...
    IMUPreintegration(const rclcpp::NodeOptions &options) : ParamServer("lio_sam_imu_preintegration", options)
    {
        ...
        callbackGroupOdom_ = create_callback_group(
            rclcpp::CallbackGroupType::MutuallyExclusive);
        auto odomOpt = rclcpp::SubscriptionOptions();
        odomOpt.callback_group = callbackGroupOdom_;
        ...
        // 发布IMU里程计数据
        pubImuOdometry_ = create_publisher<nav_msgs::msg::Odometry>(imuOdomTopic, qos_imu);
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        if (systemInitialized_ == false)
        {
            ...
            {
                if (stamp2Sec(imuQueOpt_.front().header.stamp) < currentLidarOdomTime - sync_t_)
                {
                    lastImuT_opt_ = stamp2Sec(imuQueOpt_.front().header.stamp);
            ...
            // 打印因子图和优化器
            // graphFactors_.print("GTSAM GraphFactors in systemInitialized_:\n");
            // graphValues_.print("GTSAM GraphValues in systemInitialized_:\n");
            // optimizer_.print("GTSAM optimizer_ in systemInitialized_:\n");
        ...
        while (!imuQueOpt_.empty())
        {
            ...
            sensor_msgs::msg::Imu *thisImu = &imuQueOpt_.front();
            ...
            if (imuTime < currentLidarOdomTime - sync_t_)
            {
        ...
        // 打印因子图
        // graphFactors_.print("GTSAM GraphFactors in update:\n");
        // graphValues_.print("GTSAM GraphValues in update:\n");
        // 更新一次优化器
        optimizer_.update(graphFactors_, graphValues_);
        optimizer_.update();
        // 打印更新后的优化器状态
        // optimizer_.print("GTSAM optimizer_ in update:");
        ...
        double lastImuQT = -1;
        while (!imuQueImu_.empty() && stamp2Sec(imuQueImu_.front().header.stamp) < currentLidarOdomTime - sync_t_)
        {
            lastImuQT = stamp2Sec(imuQueImu_.front().header.stamp);
        ...
        if (!imuQueImu_.empty())
        {
            ...
            for (int i = 0; i < (int)imuQueImu_.size(); ++i)
            {
                sensor_msgs::msg::Imu *thisImu = &imuQueImu_[i];
                double imuTime = stamp2Sec(thisImu->header.stamp);
    ...
    /**
     * 2. Imu数据处理回调函数
     *  这里是实际做实时的IMU里程计计算的地方，但由于预积分器都在odomHandler中被处理好了，
     *  因此这里可以直接对每一个新到来的IMU数据进行积分
     *  2.1 加锁，对新来的IMU数据放入两个队列（imuQueOpt,imuQueImu_)
     *  2.2 对IMU数据直接使用imuIntegratorImu_进行积分，并使用上一时刻的状态预测当前状态（积分结果）
     *  2.3 构建发布数据包，发布IMU里程计数据
     * 注意：
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw)
    {
        ...
        // 缓存IMU数据
        imuQueOpt_.push_back(thisImu);
        imuQueImu_.push_back(thisImu);
        ...
        gtsam::Pose3 lidarPose = imuPose.compose(imu2Lidar_);
