...
+ 同时，可以选择订阅GPS里程计，将其加入因子图，构建更准确的位姿。

订阅：
   点云特征信息集合，来自featureExtraction模块
...
#include "lio_sam_op/srv/save_map.hpp"
...
public:
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubRegisteredCurCloud_;
    // 发布回环
    rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr pubLoopConstraintEdge_;

    // 保存地图服务接口
    rclcpp::Service<lio_sam_op::srv::SaveMap>::SharedPtr srvSaveMap_;
    // 订阅从featureExtraction模块发布出来的点云信息集合
    rclcpp::Subscription<lio_sam_op::msg::CloudInfo>::SharedPtr subCloudInfo_;
    // 订阅GPS里程计（实际是由robot_localization包计算后的GPS位姿）
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr subGPS_;

    // GPS信息队列
    std::deque<nav_msgs::msg::Odometry> gpsQueue_;
    // 当前点云信息
    lio_sam_op::msg::CloudInfo cloudInfo_;

    // 所有关键帧的角点点云
    vector<pcl::PointCloud<PointType>::Ptr> cornerCloudKeyFrames_;
    // 所有关键帧的平面点点云
    vector<pcl::PointCloud<PointType>::Ptr> surfCloudKeyFrames_;
    ...
    pcl::PointCloud<PointTypePose>::Ptr copy_cloudKeyPoses6D_;

    // 当前帧的角点点云
    pcl::PointCloud<PointType>::Ptr laserCloudCornerCur_;
    // 当前帧的平面点点云
    pcl::PointCloud<PointType>::Ptr laserCloudSurfCur_;
    // 当前帧的角点点云降采样
    pcl::PointCloud<PointType>::Ptr laserCloudCornerCurDS_;
    // 当前帧的平面点点云降采样
    pcl::PointCloud<PointType>::Ptr laserCloudSurfCurDS_;
    ...
    map<int, pair<pcl::PointCloud<PointType>, pcl::PointCloud<PointType>>> laserCloudMapContainer_;
    // 局部地图角点点云（odom坐标系）
    ...
    // 构建局部地图时都挑选的关键帧做降采样
    pcl::VoxelGrid<PointType> downSizeFilterSurroundingKeyPoses_;

    // 当前雷达帧的时间戳
    rclcpp::Time timeLaserInfoStamp_;
    // 当前雷达帧的时间戳，秒
    ...
    // 降采样后局部地图角点点云数量
    int laserCloudCornerFromMapDSNum_ = 0;
    // 降采样后局部地图平面点云数量
    int laserCloudSurfFromMapDSNum_ = 0;
    // 降采样后当前帧角点点云数量
    int laserCloudCornerCurDSNum_ = 0;
    // 降采样后当前帧平面点云数量
    int laserCloudSurfCurDSNum_ = 0;

    // 当新的回环节点出现或者GPS信息被加入校正位置，这个变量被置为true，
    // 因子图优化器会执行多次更新，然后将所有的历史帧位置都更新一遍
    bool aLoopGpsIsClosed_ = false;
        ...
        : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        // 发布雷达里程计
        pubLaserOdometryGlobal_ = create_publisher<nav_msgs::msg::Odometry>(lidarOdomTopic, qos);
        // 发布全局轨迹
        pubPath_ = create_publisher<nav_msgs::msg::Path>("lio_sam/mapping/path", 1);
        // 发布回环约束
        pubLoopConstraintEdge_ = create_publisher<visualization_msgs::msg::MarkerArray>(
            "/lio_sam/mapping/loop_closure_constraints", 1);
        // 发布当前帧变换到odom坐标系下的点云
        pubRegisteredCurCloud_ = create_publisher<sensor_msgs::msg::PointCloud2>(
            "lio_sam/mapping/registered_cur_cloud", 1);
        // 发布全局地图
        pubMapCloud_ = create_publisher<sensor_msgs::msg::PointCloud2>(
            "lio_sam/mapping/map_point_cloud",
        ...
        subCloudInfo_ = create_subscription<lio_sam_op::msg::CloudInfo>(
            "lio_sam/feature/cloud_info", qos,
            std::bind(&mapOptimization::laserCloudInfoHandler, this, std::placeholders::_1));
        // 监听从robot_localization模块发布出来的GPS里程计，只有在打开GPS约束的时候才会用到这个信息
        subGPS_ = create_subscription<nav_msgs::msg::Odometry>(
            gpsTopic, 200,
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header,
                                     const std::shared_ptr<lio_sam_op::srv::SaveMap::Request> req,
                                     std::shared_ptr<lio_sam_op::srv::SaveMap::Response> res) -> void
        {
            (void)request_header;
            ...
            // 保存关键帧的transformations
            pcl::io::savePCDFileBinary(saveMapDirectory + "/trajectory.pcd", *cloudKeyPoses3D_);
            pcl::io::savePCDFileBinary(saveMapDirectory + "/transformations.pcd", *cloudKeyPoses6D_);
            // 提取全局点云地图
            ...
            for (int i = 0; i < (int)cloudKeyPoses3D_->size(); i++)
            {
                ...
                *globalSurfCloud += *transformPointCloud(surfCloudKeyFrames_[i], &cloudKeyPoses6D_->points[i]);
                cout << "\r" << std::flush << "Processing feature cloud " << i << " of " << cloudKeyPoses6D_->size() << " ...";
            }

            // 保存关键帧的角点点云，太多了，我就不保存了，想保存的话可以去掉注释就行
            // for (int i = 0; i < (int)cloudKeyPoses3D_->size(); i++)
            // {
            //     pcl::io::savePCDFileBinary(saveMapDirectory + "/kf_corner_" + std::to_string(i) + ".pcd", *cornerCloudKeyFrames_[i]);
            //     pcl::io::savePCDFileBinary(saveMapDirectory + "/kf_surf_" + std::to_string(i) + ".pcd", *surfCloudKeyFrames_[i]);
            //     cout << "\r" << std::flush << "Saving feature cloud " << i << " of " << cloudKeyPoses6D_->size() << " ...";
            // }
            ...
            if (req->resolution != 0)
            {
                ...
                downSizeFilterCorner_.setLeafSize(req->resolution, req->resolution, req->resolution);
                downSizeFilterCorner_.filter(*globalCornerCloudDS);
                pcl::io::savePCDFileBinary(saveMapDirectory + "/CornerMap.pcd", *globalCornerCloudDS);
                // down-sample and save surf cloud
                downSizeFilterSurf_.setInputCloud(globalSurfCloud);
                downSizeFilterSurf_.setLeafSize(req->resolution, req->resolution, req->resolution);
        ...
        // 地图保存service
        srvSaveMap_ = create_service<lio_sam_op::srv::SaveMap>("lio_sam/save_map", saveMapService);
        ...
        downSizeFilterSurf_.setLeafSize(mappingSurfLeafSize, mappingSurfLeafSize, mappingSurfLeafSize);
        // 回环检测时ICP匹配前点云降采样器
        downSizeFilterICP_.setLeafSize(mappingSurfLeafSize, mappingSurfLeafSize, mappingSurfLeafSize);
        // 构建局部地图时对检索出的位姿点做降采样
        downSizeFilterSurroundingKeyPoses_.setLeafSize(surroundingKeyframeDensity,
    ...
    void allocateMemory()
    {
        ...
        copy_cloudKeyPoses6D_.reset(new pcl::PointCloud<PointTypePose>());

        kdtreeSurroundingKeyPoses_.reset(new pcl::KdTreeFLANN<PointType>());
        kdtreeHistoryKeyPoses_.reset(new pcl::KdTreeFLANN<PointType>());

        laserCloudCornerCur_.reset(new pcl::PointCloud<PointType>());   // corner feature set from odoOptimization
        laserCloudSurfCur_.reset(new pcl::PointCloud<PointType>());     // surf feature set from odoOptimization
        laserCloudCornerCurDS_.reset(new pcl::PointCloud<PointType>()); // downsampled corner featuer set from odoOptimization
        laserCloudSurfCurDS_.reset(new pcl::PointCloud<PointType>());   // downsampled surf featuer set from odoOptimization

        laserCloudOri_.reset(new pcl::PointCloud<PointType>());
        coeffSel_.reset(new pcl::PointCloud<PointType>());

        laserCloudOriCornerVec_.resize(N_SCAN * Horizon_SCAN);
        coeffSelCornerVec_.resize(N_SCAN * Horizon_SCAN);
        laserCloudOriCornerFlag_.resize(N_SCAN * Horizon_SCAN);
        laserCloudOriSurfVec_.resize(N_SCAN * Horizon_SCAN);
        coeffSelSurfVec_.resize(N_SCAN * Horizon_SCAN);
        laserCloudOriSurfFlag_.resize(N_SCAN * Horizon_SCAN);
        ...
        for (int i = 0; i < 6; ++i)
        {
            transformTobeMapped_[i] = 0;
        }

        matP_.setZero();
    }

    // 处理从特征提取模块发布出来的点云特征信息，订阅特征点云的回调函数，也是整个模块核心的计算函数
    void laserCloudInfoHandler(const lio_sam_op::msg::CloudInfo::SharedPtr msgIn)
    {
        // 提取当前点云时间戳
        timeLaserInfoStamp_ = msgIn->header.stamp;
        timeLaserInfoCur_ = stamp2Sec(msgIn->header.stamp);

        // 提取当前点云的特征角点和特征平面点
        cloudInfo_ = *msgIn;
        pcl::fromROSMsg(msgIn->cloud_corner, *laserCloudCornerCur_);
        pcl::fromROSMsg(msgIn->cloud_surface, *laserCloudSurfCur_);
        ...
        if (timeLaserInfoCur_ - timeLastProcessing >= mappingProcessInterval)
        {
            timeLastProcessing = timeLaserInfoCur_;
    ...
    /**
     * GPS数据回调函数
     * liosam用的不是原始的GPS传感器出来的数据，而是经过robot_localization计算后
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void visualizeGlobalMapThread()
    {
        ...
        pcl::io::savePCDFileASCII(savePCDDirectory + "trajectory.pcd", *cloudKeyPoses3D_);
        pcl::io::savePCDFileASCII(savePCDDirectory + "transformations.pcd", *cloudKeyPoses6D_);
        pcl::PointCloud<PointType>::Ptr globalCornerCloud(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr globalCornerCloudDS(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr globalSurfCloud(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr globalSurfCloudDS(new pcl::PointCloud<PointType>());
    ...
    {
        if (pubMapCloud_->get_subscription_count() == 0)
            return;
        ...
        kdtreeGlobalMap->radiusSearch(cloudKeyPoses3D_->back(), globalMapVisualizationSearchRadius, pointSearchIndGlobalMap, pointSearchSqDisGlobalMap, 0);
        mtx_.unlock();

        for (int i = 0; i < (int)pointSearchIndGlobalMap.size(); ++i)
            globalMapKeyPoses->push_back(cloudKeyPoses3D_->points[pointSearchIndGlobalMap[i]]);
        ...
        {
            if (pointDistance(globalMapKeyPosesDS->points[i], cloudKeyPoses3D_->back()) > globalMapVisualizationSearchRadius)
                continue;
            int thisKeyInd = (int)globalMapKeyPosesDS->points[i].intensity;
            *globalMapKeyFrames += *transformPointCloud(cornerCloudKeyFrames_[thisKeyInd], &cloudKeyPoses6D_->points[thisKeyInd]);
            *globalMapKeyFrames += *transformPointCloud(surfCloudKeyFrames_[thisKeyInd], &cloudKeyPoses6D_->points[thisKeyInd]);
        }
        ...
        publishCloud(pubMapCloud_, globalMapKeyFramesDS, timeLaserInfoStamp_, odomFrame);
    }
    ...
    void performLoopClosure()
    {
        ...
        // 2. 加锁拷贝关键帧3D和6D位姿，避免多线程干扰
        mtx_.lock();
        *copy_cloudKeyPoses3D_ = *cloudKeyPoses3D_;
        *copy_cloudKeyPoses6D_ = *cloudKeyPoses6D_;
        mtx_.unlock();
        ...
        Eigen::Affine3f tWrong = pclPointToAffine3f(copy_cloudKeyPoses6D_->points[loopKeyCur]);
        // transform from world origin to corrected pose
        Eigen::Affine3f tCorrect = correctionLidarFrame * tWrong; // pre-multiplying -> successive rotation about a fixed frame
        pcl::getTranslationAndEulerAngles(tCorrect, x, y, z, roll, pitch, yaw);
        gtsam::Pose3 poseFrom = Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x, y, z));
        gtsam::Pose3 poseTo = pclPointTogtsamPose3(copy_cloudKeyPoses6D_->points[loopKeyPre]);
    ...
    bool detectLoopClosureDistance(int *latestID, int *closestID)
    {
        ...
        // 寻找空间距离相近的关键帧
        kdtreeHistoryKeyPoses_->radiusSearch(copy_cloudKeyPoses3D_->back(), historyKeyframeSearchRadius, pointSearchIndLoop, pointSearchSqDisLoop, 0);

        // 确保空间距离相近的帧是较久前采集的，排除是前面几个关键帧
        for (int i = 0; i < (int)pointSearchIndLoop.size(); ++i)
        {
            int id = pointSearchIndLoop[i];
            if (abs(copy_cloudKeyPoses6D_->points[id].time - timeLaserInfoCur_) > historyKeyframeSearchTimeDiff)
            {
                loopKeyPre = id;
                break;
            }
    ...
    void loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr &nearKeyframes, const int &key, const int &searchNum)
    {
        ...
        nearKeyframes->clear();
        int cloudSize = copy_cloudKeyPoses6D_->size();
        for (int i = -searchNum; i <= searchNum; ++i)
        {
            int keyNear = key + i;
            if (keyNear < 0 || keyNear >= cloudSize)
                continue;
            *nearKeyframes += *transformPointCloud(cornerCloudKeyFrames_[keyNear], &copy_cloudKeyPoses6D_->points[keyNear]);
            *nearKeyframes += *transformPointCloud(surfCloudKeyFrames_[keyNear], &copy_cloudKeyPoses6D_->points[keyNear]);
        }
    ...
    void visualizeLoopClosure()
    {
        ...
        markerNode.header.stamp = timeLaserInfoStamp_;
        ...
        markerEdge.header.stamp = timeLaserInfoStamp_;
        ...
        for (auto it = loopIndexContainer_.begin(); it != loopIndexContainer_.end(); ++it)
        {
            ...
            p.x = copy_cloudKeyPoses6D_->points[key_cur].x;
            p.y = copy_cloudKeyPoses6D_->points[key_cur].y;
            p.z = copy_cloudKeyPoses6D_->points[key_cur].z;
            markerNode.points.push_back(p);
            markerEdge.points.push_back(p);
            p.x = copy_cloudKeyPoses6D_->points[key_pre].x;
            p.y = copy_cloudKeyPoses6D_->points[key_pre].y;
            p.z = copy_cloudKeyPoses6D_->points[key_pre].z;
            markerNode.points.push_back(p);
            markerEdge.points.push_back(p);
        }
    ...
    void updateInitialGuess()
    {
        ...
        {
            RCLCPP_INFO(get_logger(), "cloudKeyPoses3D_ is empty, set transform init to imu rotate");
            transformTobeMapped_[0] = cloudInfo_.imu_roll_init;
            transformTobeMapped_[1] = cloudInfo_.imu_pitch_init;
            transformTobeMapped_[2] = cloudInfo_.imu_yaw_init;

            if (!useImuHeadingInitialization)
                transformTobeMapped_[2] = 0;

            lastImuTransformation = pcl::getTransformation(0, 0, 0, cloudInfo_.imu_roll_init, cloudInfo_.imu_pitch_init, cloudInfo_.imu_yaw_init); // save imu before return;
            return;
        }
        ...
        if (cloudInfo_.odom_available == true)
        {
            Eigen::Affine3f transBack = pcl::getTransformation(
                cloudInfo_.initial_guess_x, cloudInfo_.initial_guess_y, cloudInfo_.initial_guess_z,
                cloudInfo_.initial_guess_roll, cloudInfo_.initial_guess_pitch, cloudInfo_.initial_guess_yaw);
            ...
            else
            {
                ...
                Eigen::Affine3f transTobe = trans2Affine3f(transformTobeMapped_);
                Eigen::Affine3f transFinal = transTobe * transIncre;
                pcl::getTranslationAndEulerAngles(transFinal, transformTobeMapped_[3], transformTobeMapped_[4], transformTobeMapped_[5],
        ...
        if (cloudInfo_.imu_available == true)
        {
            Eigen::Affine3f transBack = pcl::getTransformation(0, 0, 0, cloudInfo_.imu_roll_init, cloudInfo_.imu_pitch_init, cloudInfo_.imu_yaw_init);
    ...
    void extractNearby()
    {
        ...
        kdtreeSurroundingKeyPoses_->radiusSearch(cloudKeyPoses3D_->back(), (double)surroundingKeyframeSearchRadius, pointSearchInd, pointSearchSqDis);
        ...
        int numPoses = cloudKeyPoses3D_->size();
        for (int i = numPoses - 1; i >= 0; --i)
        {
            if (timeLaserInfoCur_ - cloudKeyPoses6D_->points[i].time < 10.0)
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract)
    {
        ...
        {
            if (pointDistance(cloudToExtract->points[i], cloudKeyPoses3D_->back()) > surroundingKeyframeSearchRadius)
            {
                RCLCPP_INFO(get_logger(), "pointDistance too large! Empty laserCloudCornerFromMap_ and laserCloudSurfFromMap_");
                continue;
            }
            ...
            else
            {
                ...
                pcl::PointCloud<PointType> laserCloudCornerTemp = *transformPointCloud(cornerCloudKeyFrames_[thisKeyInd], &cloudKeyPoses6D_->points[thisKeyInd]);
                pcl::PointCloud<PointType> laserCloudSurfTemp = *transformPointCloud(surfCloudKeyFrames_[thisKeyInd], &cloudKeyPoses6D_->points[thisKeyInd]);
    ...
    {
        // 降采样当前帧角点点云
        laserCloudCornerCurDS_->clear();
        downSizeFilterCorner_.setInputCloud(laserCloudCornerCur_);
        downSizeFilterCorner_.filter(*laserCloudCornerCurDS_);
        laserCloudCornerCurDSNum_ = laserCloudCornerCurDS_->size();

        // 降采样当前帧平面点点云
        laserCloudSurfCurDS_->clear();
        downSizeFilterSurf_.setInputCloud(laserCloudSurfCur_);
        downSizeFilterSurf_.filter(*laserCloudSurfCurDS_);
        laserCloudSurfCurDSNum_ = laserCloudSurfCurDS_->size();
    }
...
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < laserCloudCornerCurDSNum_; i++)
        {
            ...
            pointOri = laserCloudCornerCurDS_->points[i];
...
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < laserCloudSurfCurDSNum_; i++)
        {
            PointType pointOri, pointSel, coeff;
            std::vector<int> pointSearchInd;
            std::vector<float> pointSearchSqDis;

            pointOri = laserCloudSurfCurDS_->points[i];
            pointAssociateToMap(&pointOri, &pointSel);
    ...
    void combineOptimizationCoeffs()
    {
        ...
        for (int i = 0; i < laserCloudCornerCurDSNum_; ++i)
        {
            if (laserCloudOriCornerFlag_[i] == true)
            {
        ...
        for (int i = 0; i < laserCloudSurfCurDSNum_; ++i)
        {
            if (laserCloudOriSurfFlag_[i] == true)
            {
    ...
    bool LMOptimization(int iterCount)
    {
        ...
        cv::Mat matP_(6, 6, CV_32F, cv::Scalar::all(0));
    ...
    void scan2MapOptimization()
    {
        ...
        if (laserCloudCornerCurDSNum_ > edgeFeatureMinValidNum && laserCloudSurfCurDSNum_ > surfFeatureMinValidNum)
        {
        ...
        {
            RCLCPP_WARN(get_logger(), "Not enough features! Only %d edge and %d planar features available.", laserCloudCornerCurDSNum_, laserCloudSurfCurDSNum_);
        }
    }
    ...
    void transformUpdate()
    {
        if (cloudInfo_.imu_available == true)
        {
            if (std::abs(cloudInfo_.imu_pitch_init) < 1.4)
            {
                ...
                imuQuaternion.setRPY(cloudInfo_.imu_roll_init, 0, 0);
                ...
                imuQuaternion.setRPY(0, cloudInfo_.imu_pitch_init, 0);
                tf2::Matrix3x3(transformQuaternion.slerp(imuQuaternion, imuWeight)).getRPY(rollMid, pitchMid, yawMid);
                transformTobeMapped_[1] = pitchMid;
            }
        }

        transformTobeMapped_[0] = constraintTransformation(transformTobeMapped_[0], rotation_tollerance);
        transformTobeMapped_[1] = constraintTransformation(transformTobeMapped_[1], rotation_tollerance);
        transformTobeMapped_[5] = constraintTransformation(transformTobeMapped_[5], z_tollerance);
    ...
    bool saveFrame()
    {
        ...
        Eigen::Affine3f transStart = pclPointToAffine3f(cloudKeyPoses6D_->back());
        Eigen::Affine3f transFinal = pcl::getTransformation(transformTobeMapped_[3], transformTobeMapped_[4], transformTobeMapped_[5],
                                                            transformTobeMapped_[0], transformTobeMapped_[1], transformTobeMapped_[2]);
    ...
    void addOdomFactor()
    {
        if (cloudKeyPoses3D_->points.empty())
        {
            ...
            gtSAMgraph_.add(PriorFactor<Pose3>(0, trans2gtsamPose(transformTobeMapped_), priorNoise));
            initialEstimate_.insert(0, trans2gtsamPose(transformTobeMapped_));
        }
        else
        {
            noiseModel::Diagonal::shared_ptr odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-6, 1e-6, 1e-6, 1e-4, 1e-4, 1e-4).finished());
            gtsam::Pose3 poseFrom = pclPointTogtsamPose3(cloudKeyPoses6D_->points.back());
            gtsam::Pose3 poseTo = trans2gtsamPose(transformTobeMapped_);
            gtSAMgraph_.add(BetweenFactor<Pose3>(cloudKeyPoses3D_->size() - 1, cloudKeyPoses3D_->size(), poseFrom.between(poseTo), odometryNoise));
    ...
    void addGPSFactor()
    {
        ...
        {
            if (pointDistance(cloudKeyPoses3D_->front(), cloudKeyPoses3D_->back()) < 5.0)
                return;
        }

        // pose covariance small, no need to correct
        if (poseCovariance_(3, 3) < poseCovThreshold && poseCovariance_(4, 4) < poseCovThreshold)
            return;
        ...
        {
            if (stamp2Sec(gpsQueue_.front().header.stamp) < timeLaserInfoCur_ - 0.2)
            {
                // message too old
                gpsQueue_.pop_front();
            }
            else if (stamp2Sec(gpsQueue_.front().header.stamp) > timeLaserInfoCur_ + 0.2)
            {
                // message too new
                break;
            }
            ...
            {
                nav_msgs::msg::Odometry thisGPS = gpsQueue_.front();
                gpsQueue_.pop_front();
                ...
                gtsam::GPSFactor gps_factor(cloudKeyPoses3D_->size(), gtsam::Point3(gps_x, gps_y, gps_z), gps_noise);
                gtSAMgraph_.add(gps_factor);

                aLoopGpsIsClosed_ = true;
                break;
            }
        }
    }
    ...
    void addLoopFactor()
    {
        ...
        for (int i = 0; i < (int)loopIndexQueue_.size(); ++i)
        {
            ...
            gtSAMgraph_.add(BetweenFactor<Pose3>(indexFrom, indexTo, poseBetween, noiseBetween));
        }
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        latestEstimate = isamCurrentEstimate_.at<Pose3>(isamCurrentEstimate_.size() - 1);
        // cout << "****************************************************" << endl;
        // isamCurrentEstimate_.print("Current estimate: ");

        // 将当前帧经过优化的结果存入关键帧位姿序列（3D/6D）
        thisPose3D.x = latestEstimate.translation().x();
        thisPose3D.y = latestEstimate.translation().y();
        thisPose3D.z = latestEstimate.translation().z();
        thisPose3D.intensity = cloudKeyPoses3D_->size(); // this can be used as index
        ...
        thisPose6D.time = timeLaserInfoCur_;
        cloudKeyPoses6D_->push_back(thisPose6D);

        // cout << "****************************************************" << endl;
        // cout << "Pose covariance:" << endl;
        // cout << isam_->marginalCovariance(isamCurrentEstimate_.size()-1) << endl << endl;
        poseCovariance_ = isam_->marginalCovariance(isamCurrentEstimate_.size() - 1);

        // 将当前帧经过优化的结果更新到全局缓存变量
        transformTobeMapped_[0] = latestEstimate.rotation().roll();
        transformTobeMapped_[1] = latestEstimate.rotation().pitch();
        transformTobeMapped_[2] = latestEstimate.rotation().yaw();
        transformTobeMapped_[3] = latestEstimate.translation().x();
        transformTobeMapped_[4] = latestEstimate.translation().y();
        transformTobeMapped_[5] = latestEstimate.translation().z();

        // 将当前帧的角点、平面点点云存入关键帧点云缓存队列
        pcl::PointCloud<PointType>::Ptr thisCornerKeyFrame(new pcl::PointCloud<PointType>());
        pcl::PointCloud<PointType>::Ptr thisSurfKeyFrame(new pcl::PointCloud<PointType>());
        pcl::copyPointCloud(*laserCloudCornerCurDS_, *thisCornerKeyFrame);
        pcl::copyPointCloud(*laserCloudSurfCurDS_, *thisSurfKeyFrame);
        // save key frame cloud
        cornerCloudKeyFrames_.push_back(thisCornerKeyFrame);
        surfCloudKeyFrames_.push_back(thisSurfKeyFrame);
    ...
    void correctPoses()
    {
        ...
        if (aLoopGpsIsClosed_ == true)
        {
            ...
            {
                cloudKeyPoses3D_->points[i].x = isamCurrentEstimate_.at<Pose3>(i).translation().x();
                cloudKeyPoses3D_->points[i].y = isamCurrentEstimate_.at<Pose3>(i).translation().y();
                cloudKeyPoses3D_->points[i].z = isamCurrentEstimate_.at<Pose3>(i).translation().z();
                ...
                cloudKeyPoses6D_->points[i].roll = isamCurrentEstimate_.at<Pose3>(i).rotation().roll();
                cloudKeyPoses6D_->points[i].pitch = isamCurrentEstimate_.at<Pose3>(i).rotation().pitch();
                cloudKeyPoses6D_->points[i].yaw = isamCurrentEstimate_.at<Pose3>(i).rotation().yaw();

                updatePath(cloudKeyPoses6D_->points[i]);
            }

            aLoopGpsIsClosed_ = false;
        }
    }
    ...
    void publishOdometry()
    {
        ...
        laserOdometryROS.header.stamp = timeLaserInfoStamp_;
        ...
        if (lastIncreOdomPubFlag == false)
        {
            ...
            increOdomAffine = trans2Affine3f(transformTobeMapped_);
        }
        else
        {
            /**
             ...
             * 也就是说，incremental的激光里程计没有使用到因子图优化的结果，因此更加光滑，没有跳变，可以给IMU预积分模块使用
             */
            Eigen::Affine3f affineIncre = incrementalOdometryAffineFront_.inverse() * incrementalOdometryAffineBack_;
            ...
            laserOdomIncremental.header.stamp = timeLaserInfoStamp_;
        ...
        // pubLaserOdometryGlobal_->publish(laserOdomIncremental);
    }
    ...
    void publishFrames()
    {
        ...
        if (pubRegisteredCurCloud_->get_subscription_count() != 0)
        {
            pcl::PointCloud<PointType>::Ptr cloudOut(new pcl::PointCloud<PointType>());
            PointTypePose thisPose6D = trans2PointTypePose(transformTobeMapped_);
            *cloudOut += *transformPointCloud(laserCloudCornerCurDS_, &thisPose6D);
            *cloudOut += *transformPointCloud(laserCloudSurfCurDS_, &thisPose6D);
            publishCloud(pubRegisteredCurCloud_, cloudOut, timeLaserInfoStamp_, odomFrame);
        }
